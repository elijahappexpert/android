...
/** EBHIncidentProvider:
 * 
 * This class handles all of the data interactions dealing with the Epidemic and 
 * Biological Hazard Incident segment of this application. It has full knowledge 
 * of the underlying Database, which IS different than the structure of the EBHIncident 
 * objects themselves.
 *
 */
public class EBHIncidentProvider {
	
	// Used to get the Log.D TAG from the strings.XML file so its flexible
	private static final String TAG = EBHIncidentProvider.class.getSimpleName();
	...
	// Initialize Class Wide objects ...
	   Context context;
	   EbhDBHelper ebhdbHelper; 
	   SQLiteDatabase db;
	
		// DB Constants ...
		public static final String DATABASE_NAME = "ebhincident.db";
		public static final int DATABASE_VERSION = 1;
		public static final String EBHINCIDENT_TABLE = "ebhincident";
		...
		public static final int CDATA_COLUMN = 6;
		public static final int LINK_COLUMN = 7;
		  
		// Database Create string Constant ...
	    private static final String DATABASE_CREATE =
	      "create table " + EBHINCIDENT_TABLE + " (" 
		...
		// Constructor ....
		public EBHIncidentProvider(Context context) {
			
			this.context = context;     		// Get the context ...	
			ebhdbHelper = new EbhDBHelper();	// Get DBHelper object, 
		 ...
		 */
		public Cursor query() {
			db = ebhdbHelper.getReadableDatabase();		// Get the database ...	
			...
			return db.query(EBHIncidentProvider.EBHINCIDENT_TABLE, null, null, null, null, null, KEY_DATE + " DESC");		   		
		...
		public long insert(ContentValues nv_pairs) {
			...
			db = ebhdbHelper.getWritableDatabase();
			...
			long ret;
			try {
				ret = db.insertOrThrow(EBHIncidentProvider.EBHINCIDENT_TABLE, null, nv_pairs);
		 ...
		 * 
		 * 	@param [EBHIncident ebhincident] :=> (EBHIncident data as provided by Online XML Feed)
		 * 	Takes the EBHIncident data from our XML data feed and creates a new ContentValues
		 * 	(name/value pairs) object, inserts the EBHIncident data into that object and 
		 * 	then calls the 'other' INSERT method, which knows how to insert that type of data
		 * 	into the underlying database. 
		 */		
		public long insert(EBHIncident ebhincident) {
		    ...
		    nv_pairs.put(EBHIncidentProvider.KEY_ID, ebhincident.getId());
		    nv_pairs.put(EBHIncidentProvider.KEY_DATE, ebhincident.getDate().getTime());
		    nv_pairs.put(EBHIncidentProvider.KEY_DETAILS, ebhincident.getDetails());
		    nv_pairs.put(EBHIncidentProvider.KEY_DESCRIPTION, ebhincident.getDescription());
		    nv_pairs.put(EBHIncidentProvider.KEY_GEOPOINT_LAT, ebhincident.getGeoPointLat());
		    nv_pairs.put(EBHIncidentProvider.KEY_GEOPOINT_LNG, ebhincident.getGeoPointLng());
		    nv_pairs.put(EBHIncidentProvider.KEY_CDATA, ebhincident.getCData());
		    nv_pairs.put(EBHIncidentProvider.KEY_LINK, ebhincident.getLink());
		 ...
		 */
		public void delete () {
			db = ebhdbHelper.getWritableDatabase();	// getWritableDatabase
			// Delete the actual data ...
			db.delete(EBHIncidentProvider.EBHINCIDENT_TABLE, null, null);
			db.close(); // Close Database 
		}
		 ...
		 */
		private class EbhDBHelper extends SQLiteOpenHelper {
				
		    // Constructor ...	
			public EbhDBHelper() {
		      super(context, EBHIncidentProvider.DATABASE_NAME, null, EBHIncidentProvider.DATABASE_VERSION);
		    }

		    
		    @Override
		    public void onCreate(SQLiteDatabase db) {
		      db.execSQL(EBHIncidentProvider.DATABASE_CREATE);           
		    }
		    ...
		    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		    	...
		    	db.execSQL("DROP TABLE IF EXISTS " + EBHIncidentProvider.EBHINCIDENT_TABLE);
		    	onCreate(db);
		    }
